...
function wiresCanvas() {
    return document.getElementById('wires-layer');
}
export function getWorkspace() {
    return document.getElementById('node-workspace');
}
...
export function registerWire(fromEl, toEl, color, dimmed) {
    wireRegistry.push({ fromEl, toEl, color, dimmed });
}
export function updateWireDimming(query) {
    for (const w of wireRegistry) {
        const name = w.toEl._itemName ?? '';
        w.dimmed = !!query && !name.toLowerCase().includes(query.toLowerCase());
    }
}
export function redrawWires() {
    const c = wiresCanvas();
    const ctx = c.getContext('2d');
    ctx.clearRect(0, 0, c.width, c.height);
    const nodeWidth = 200;
